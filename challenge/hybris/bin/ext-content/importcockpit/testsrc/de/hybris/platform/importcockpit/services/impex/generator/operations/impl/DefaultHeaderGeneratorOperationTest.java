...
public class DefaultHeaderGeneratorOperationTest
{
	...
	public void setUp()
	{
		...
		when(atomicMappingLineModel.getChildren()).thenReturn(Collections.singletonList(subLineAtomicNonLocalized));
		...
		when(enumLineLocalized.getChildren()).thenReturn(Collections.singletonList(enumLineNonLocalized));
	...
	public void testGenerateAttributes4ProductHeaderMissingId()
	{
		...
		when(attributeLine.getChildren()).thenReturn(Collections.singletonList(translatorMappingLine));
	...
	{
		assertThat(operation.generateImportBeanShellStatement(TEST_FILE_NAME))
				.isEqualTo("#% impex.includeExternalDataMedia(\"" + TEST_FILE_NAME + ".csv\", \"UTF-8\", ';', 0, -1);");
	}
	...
	{
		when(mappingModel.getImpexImportMode()).thenReturn(ImpexImportMode.INSERT);
		when(mappingModel.getEntries()).thenReturn(Lists.newArrayList(subLineAtomicNonLocalized, subLineAtomicLocalized,
				subLineNonAtomicLocalized, attributeLine, enumLineNonLocalized, enumLineLocalized));
	...
	{
		final HashSet<MappingLineModel> mappingLines = Sets.newLinkedHashSet();
	...
	{
		when(composedMappingLineModel.getChildren()).thenReturn(Collections.singletonList(subLineAtomicLocalized));
	...
	public void testGenerateImpexSubHeaders()
	{
		...
		when(composedMappingLineModel.getMapping()).thenReturn(mappingModel);
		final List<Map<Integer, String>> lines = operation.generateImpexSubHeaders(Sets.newHashSet(composedMappingLineModel));
		...
		assertThat(lines.get(1).get(Integer.valueOf(0)))
				.isEqualTo("#% impex.includeExternalDataMedia(\"Product.csv\", \"UTF-8\", ';', 0, -1);");
		assertThat(lines.get(2).size()).isEqualTo(0);
	}

	@Test
	public void isCatalogVersionManuallySet()
	{
		Mockito.doReturn(Boolean.FALSE).when(mappingModel).isCatalogVersionOption();
		assertFalse(operation.isCatalogVersionManuallySet(mappingModel));

		Mockito.doReturn(Boolean.TRUE).when(mappingModel).isCatalogVersionOption();
		assertFalse(operation.isCatalogVersionManuallySet(mappingModel));

		final MappingLineModel line = mock(MappingLineModel.class);
		when(line.getAttributeCode()).thenReturn(CV);
		when(mappingModel.getEntries()).thenReturn(Collections.singletonList(line));
		assertTrue(operation.isCatalogVersionManuallySet(mappingModel));
	}
}
