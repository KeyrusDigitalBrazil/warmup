...
public class BMECatParser
{
	...
	private void init(final BMECatObjectProcessor processor, final Map props, final BMECatContentHandler handler)
	{
		...
		final Map features = obtainDefaultFeatures();
		if (props != null)
		{
	...
	public void setFeatures(final Map props)
	{
		...
		{
			System.err.println(
					"warning: Parser does not support feature (" + BMECatConstants.PARSER.NAMESPACE_PREFIXES_FEATURE_ID + ")");
		}
		try
		{
		...
		{
			System.err.println(
					"warning: Parser does not recognize feature (" + BMECatConstants.PARSER.LOAD_EXTERNAL_DTD_FEATURE_ID + ")");
		}
		catch (final SAXNotSupportedException e)
		{
			System.err
					.println("warning: Parser does not support feature (" + BMECatConstants.PARSER.LOAD_EXTERNAL_DTD_FEATURE_ID + ")");
		}
		try
		{
		...
		{
			System.err.println(
					"warning: Parser does not recognize feature (" + BMECatConstants.PARSER.SCHEMA_VALIDATION_FEATURE_ID + ")");
		}
		catch (final SAXNotSupportedException e)
		{
			System.err
					.println("warning: Parser does not support feature (" + BMECatConstants.PARSER.SCHEMA_VALIDATION_FEATURE_ID + ")");
		}
		try
		{
		...
		{
			System.err.println(
					"warning: Parser does not recognize feature (" + BMECatConstants.PARSER.SCHEMA_FULL_CHECKING_FEATURE_ID + ")");
		}
		catch (final SAXNotSupportedException e)
		{
			System.err.println(
					"warning: Parser does not support feature (" + BMECatConstants.PARSER.SCHEMA_FULL_CHECKING_FEATURE_ID + ")");
		}
		try
		{
		...
		{
			System.err.println(
					"warning: Parser does not recognize feature (" + BMECatConstants.PARSER.DYNAMIC_VALIDATION_FEATURE_ID + ")");
		}
		catch (final SAXNotSupportedException e)
		{
			System.err.println(
					"warning: Parser does not support feature (" + BMECatConstants.PARSER.DYNAMIC_VALIDATION_FEATURE_ID + ")");
		}
		try
		{
			if (props.get(BMECatConstants.PARSER.LOAD_EXTERNAL_DTD_FEATURE_ID) != null)
		...
		try
		{
			if (props.get(BMECatConstants.PARSER.EXTERNAL_PARAMETER_ENTITIES) != null)
			{
				parser.setFeature(BMECatConstants.PARSER.EXTERNAL_PARAMETER_ENTITIES,
						props.get(BMECatConstants.PARSER.EXTERNAL_PARAMETER_ENTITIES).equals(Boolean.TRUE.toString()) ? true : false);
			}
		}
		catch (final SAXNotRecognizedException e)
		{
			System.err.println(
					"warning: Parser does not recognize feature (" + BMECatConstants.PARSER.EXTERNAL_PARAMETER_ENTITIES + ")");
		}
		catch (final SAXNotSupportedException e)
		{
			System.err
					.println("warning: Parser does not support feature (" + BMECatConstants.PARSER.EXTERNAL_PARAMETER_ENTITIES + ")");
		}
		try
		{
			if (props.get(BMECatConstants.PARSER.EXTERNAL_GENERAL_ENTITIES) != null)
			{
				parser.setFeature(BMECatConstants.PARSER.EXTERNAL_GENERAL_ENTITIES,
						props.get(BMECatConstants.PARSER.EXTERNAL_GENERAL_ENTITIES).equals(Boolean.TRUE.toString()) ? true : false);
			}
		}
		catch (final SAXNotRecognizedException e)
		{
			System.err
					.println("warning: Parser does not recognize feature (" + BMECatConstants.PARSER.EXTERNAL_GENERAL_ENTITIES + ")");
		}
		catch (final SAXNotSupportedException e)
		{
			System.err
					.println("warning: Parser does not support feature (" + BMECatConstants.PARSER.EXTERNAL_GENERAL_ENTITIES + ")");
		}
		try
		{
			if (props.get(BMECatConstants.PARSER.DISALLOW_DOCTYPE_DECL) != null)
			{
				parser.setFeature(BMECatConstants.PARSER.DISALLOW_DOCTYPE_DECL,
						props.get(BMECatConstants.PARSER.DISALLOW_DOCTYPE_DECL).equals(Boolean.TRUE.toString()) ? true : false);
			}
		}
		catch (final SAXNotRecognizedException e)
		{
			System.err.println("warning: Parser does not recognize feature (" + BMECatConstants.PARSER.DISALLOW_DOCTYPE_DECL + ")");
		}
		catch (final SAXNotSupportedException e)
		{
			System.err.println("warning: Parser does not support feature (" + BMECatConstants.PARSER.DISALLOW_DOCTYPE_DECL + ")");
		}
	}
	...
	protected Map obtainDefaultFeatures()
	{
		...
		props.put(BMECatConstants.PARSER.DISALLOW_DOCTYPE_DECL, Boolean.TRUE.toString());
		props.put(BMECatConstants.PARSER.EXTERNAL_GENERAL_ENTITIES, Boolean.FALSE.toString());
		props.put(BMECatConstants.PARSER.EXTERNAL_PARAMETER_ENTITIES, Boolean.FALSE.toString());
		return props;
	}
