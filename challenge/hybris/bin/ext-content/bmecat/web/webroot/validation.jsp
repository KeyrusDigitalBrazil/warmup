...
<% 
	...
	out.write("script: " + XSSEncoder.encodeHTML(scriptConstants) + XSSEncoder.encodeHTML(script) + "<br/>");
	...
	try
	{
		...
		out.print( "<br>Starting import synchronous using " + XSSEncoder.encodeHTML(cronJobString) + "<br>" );
		...
		finally
		{
			...
			{
				out.print("WARNING: Can not remove log listener for cronjob " + XSSEncoder.encodeHTML(cronJobString) );
			}
		}
		if( cronJob==null )
		{
			out.print( "<br><font color='green'>Import was successful (using " + XSSEncoder.encodeHTML(cronJobString) + ")</font><br>" );
		}
		else
		{
			out.print( "<br><font color='red'>The content to be imported has caused an error, please refer to the CronJob with code " + XSSEncoder.encodeHTML(cronJob.getCode()) +
			(cronJob.getUnresolvedDataStore() == null ?
			" (no lines were dumped) " :
			" as well as the CronJob's dump file with code " + XSSEncoder.encodeHTML(cronJob.getUnresolvedDataStore().getCode()) +  " for details.</font><br>" ));
		}
	}
	catch( ImpExException hve)
	{
		//XXX unknown type, exception caught in console, but not in program?
		out.write("unknown type on screen: " + XSSEncoder.encodeHTML(hve.getMessage()));
