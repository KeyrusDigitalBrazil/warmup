	...
	while( iter.hasNext() )
	{
			...
			if( !fis[0].isFormField() )
			{
				...
				if(importFileName.toLowerCase().endsWith(".xml"))
				{
					...
					try
					{
						...
						{
							out.print("<a href=\"" + XSSEncoder.encodeHTML(media.getDownloadURL())+ "\">zip_file </a><br/>");
						}
						out.print("<br/>Info of transformed catalog:<br/>");
						out.print("<br/>" + XSSEncoder.encodeHTML(proc.getGeneralCatalogInfo()) + "<br/>");
						...
						if(bmecatHeader.containsKey("catalog.id"))
						{
							...
							out.print("$catalog_id=" + XSSEncoder.encodeHTML(bmecatHeader.get("catalog.id")) + "<br/>");
						}
						if(bmecatHeader.containsKey("catalog.version"))
						{
							scriptConstants += "$catalog_version=" + bmecatHeader.get("catalog.version") + "\n";
							out.print("$catalog_version=" + XSSEncoder.encodeHTML(bmecatHeader.get("catalog.version")) + "<br/>");
						}
						if(bmecatHeader.containsKey("catalog.name"))
						{
							scriptConstants += "$catalog_name=" + bmecatHeader.get("catalog.name") + "\n";
							out.print("$catalog_name=" + XSSEncoder.encodeHTML(bmecatHeader.get("catalog.name")) + "<br/>");
						}
						...
						if(bmecatHeader.containsKey("catalog.currency"))
						{
							...
							out.print("$cur_iso=" + XSSEncoder.encodeHTML(bmecatHeader.get("catalog.currency")) + "<br/>");
					...
					out.print("<br/>File [" + XSSEncoder.encodeHTML(importFileName) + "] is transformed to csv files successfully.<br/>");
				}
				else
				{
					out.print("<br/><font color='red'>The first file [" + XSSEncoder.encodeHTML(importFileName) + "] is not valid.</font><br/>");
		...
		else if(i == 1)
		{
			...
			if( !fis[1].isFormField() )
			{
				...
				{
					out.print("<br/><font color='red'>The second file [" + XSSEncoder.encodeHTML(importFileName) + "] is not valid.</font><br/>");
		...
		<tr>
			<td>
				...
				<form name="bmecatImportForm" action="validation.jsp" method="post">
					<input type="hidden" name="mediaPK" value="<%= XSSEncoder.encodeHTML(mediaPK) %>"></input>
					<input type="hidden" name="scriptConstants" value="<%= XSSEncoder.encodeHTML(scriptConstants) %>"></input>
					<input type="hidden" name="resourceMediaPK" value="<%= XSSEncoder.encodeHTML(resourceMediaPK) %>"></input>
					<textarea name="script" cols="100" rows="20" wrap="off"><%=XSSEncoder.encodeHTML(script)%></textarea>
					<p/>
