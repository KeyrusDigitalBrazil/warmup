...
module.exports = function() {
    ...
    function getFeatures(done) {
        return browser.get('jsTests/tests/personalizationsmarteditcontainer/e2e/features/app.html').then(function() {
            if (done) {
                done();
            }
        });
    ...
    function getE2EConfigs() {
        var configDirs = getConfigDirs(PATHS.tests.personalizationsmarteditContainere2eTestsRoot);
        var configs = [];
        configDirs.forEach(function(configDir) {
            ...
            if (testFile) {
                ...
                try {
                    var path = configDir.substr(PATHS.tests.personalizationsmarteditContainere2eTestsRoot.length);
                    configs.push({
                        key: path + '/' + testFile,
    ...
    function getTestsListPageHtml(e2eConfigs) {
        var tplFileContent = fs.readFileSync(PATHS.tests.personalizationsmarteditContainere2eTestsRoot + '/features/list.html.tpl', 'utf8');
    ...
    function createTestsListPage() {
        ...
        var fileName = PATHS.tests.personalizationsmarteditContainere2eTestsRoot + '/features/list.html';
    ...
    return {
        loadE2eDependencies: function(browser, dependencyContainer) {
             ...
             * /personalizationsmartedit/jsTests/tests/personalizationsmarteditcontainer/e2e/features/util/commonMockedModule/configurationMocks.js
             */
