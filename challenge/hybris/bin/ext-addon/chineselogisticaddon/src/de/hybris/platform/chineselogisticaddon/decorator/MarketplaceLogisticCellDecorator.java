 ...
 *
 */
public class MarketplaceLogisticCellDecorator implements CSVCellDecorator
{
	private static final Logger LOG = Logger.getLogger(MarketplaceLogisticCellDecorator.class);
	...
	public String decorate(final int position, final Map<Integer, String> impexLine)
	{
		...
		final StringBuilder queryBuilder = new StringBuilder("SELECT {pk} FROM {ContentSlot} WHERE {uid} =?uid AND ");
		...
		try
		{
			catalogVersion = getCatalogVersionService().getCatalogVersion("marketplaceContentCatalog", "Staged");
		}
		...
		{
			LOG.debug("Initialization:" + MarketplaceLogisticCellDecorator.class.getName() + " found components are "
