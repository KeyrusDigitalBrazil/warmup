...
<%@ taglib prefix="cartaddon" tagdir="/WEB-INF/tags/addons/sapsubscriptionaddon/responsive/cart" %>
        ...
        <c:otherwise>
            <c:if test="${not empty entryGroup.orderEntries}">
                <c:forEach items="${entryGroup.orderEntries}" var="entry">
                    <tr>
                        <td>
                            <cartaddon:cartItem cartData="${cartData}" entry="${entry}" index="${entryGroup.groupNumber}"/>
                        </td>
                    </tr>
                </c:forEach>
            </c:if>
