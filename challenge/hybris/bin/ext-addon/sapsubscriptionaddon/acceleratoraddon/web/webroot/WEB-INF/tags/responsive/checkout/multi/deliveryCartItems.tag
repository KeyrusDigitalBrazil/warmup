			...
			<div class="thumb">
				...
				<c:choose>
					<c:when test="${not empty entry.product.subscriptionTerm}">
						<c:forEach items="${entry.orderEntryPrices}"
							var="orderEntryPrice">
							<!-- please ensure that these TDs get rendered always to avoid a uneven number of TDs per Row -->
							<c:if
								test='${orderEntryPrice.billingTime ne null and orderEntryPrice.billingTime.code eq "paynow"}'>
								<div class="price">
									<format:price priceData="${orderEntryPrice.basePrice}" displayFreeForZero="true" />
									<div>${orderEntryPrice.billingTime.name}</div>
								</div>
		                    ...
		                    <c:when test="${not empty entry.product.subscriptionTerm}">
		                        <c:forEach items="${entry.orderEntryPrices}" var="orderEntryPrice"> <!-- please ensure that these TDs get rendered always to avoid a uneven number of TDs per Row -->
		                            <span class="visible-xs visible-sm"><spring:theme code="basket.page.itemPrice"/>: </span>
							  		<c:if test = "${orderEntryPrice.billingTime ne null and orderEntryPrice.billingTime.code ne 'paynow'}">
										<format:price priceData="${orderEntryPrice.basePrice}" displayFreeForZero="true" />&nbsp;
