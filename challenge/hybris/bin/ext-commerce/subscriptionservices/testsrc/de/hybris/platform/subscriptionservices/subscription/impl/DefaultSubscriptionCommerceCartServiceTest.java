...
public class DefaultSubscriptionCommerceCartServiceTest
{
	...
	public void setUp() throws Exception
	{
		...
		given(Boolean.valueOf(subscriptionProductService.isSubscription(subscriptionProductModel))).willReturn(Boolean.TRUE);
		given(Boolean.valueOf(subscriptionProductService.isSubscription(productModel))).willReturn(Boolean.FALSE);
		...
		final List<AbstractOrderModel> childCarts = new ArrayList<>();
	...
	public void testUpdateQuantityForCartEntrySubscriptionProduct() throws CommerceCartModificationException, CalculationException
	{
		...
		given(masterCartModel.getEntries()).willReturn(Collections.singletonList(entryToUpdate));
		given(masterCartModel.getBillingTime()).willReturn(billingFrequencyModelPayNow);
		given(Boolean.valueOf(calculationService.requiresCalculation(masterCartModel))).willReturn(Boolean.TRUE);
	...
	public void testCheckMasterCartIsNoMasterCart() throws CommerceCartModificationException
	{
		...
		commerceCartService.checkMasterCart(childCartModelMonthly, billingFrequencyModelPayNow);
	}
	...
	public void testCheckMasterCartMasterHasWrongBillingFrequency() throws CommerceCartModificationException
	{
		...
		commerceCartService.checkMasterCart(masterCartModel, billingFrequencyModelPayNow);
	}
	...
	public void testCreatingFirstChildCart()
	{
		...
		cart.setChildren(null);
		given(modelService.create(CartModel.class)).willReturn(new CartModel());

		final CartModel childCart = commerceCartService.createChildCartForBillingTime(cart, billingFrequencyModelMonthly);

		assertNotNull(childCart);
		assertThat(cart.getChildren(), contains(childCart));
	...
	{
		final CartModel cart = new CartModel();
		final CartModel child1 = new CartModel();
		cart.setChildren(Collections.singletonList(child1));
		given(modelService.create(CartModel.class)).willReturn(new CartModel());

		final CartModel childCart = commerceCartService.createChildCartForBillingTime(cart, billingFrequencyModelMonthly);

		assertNotNull(childCart);
		assertThat(cart.getChildren(), containsInAnyOrder(child1, childCart));
	}
