...
public class XmlAssertions
{
	...
	{
		final String expected = transformation == null ? expectedValue
				: XmlUtils.applyTransformationResource(expectedValue, transformation);
	...
	/**
	 * Asserts that two XML snippets are equal. Before comparing the given XSL transformation is applied to the expected as
	 * well as the actual XML.
	 *
	 * @param message
	 *           failure message
	 * @param expectedXml
	...
	{
		final String expected = transformation == null ? expectedXml
				: XmlUtils.applyTransformationResource(expectedXml, transformation);
