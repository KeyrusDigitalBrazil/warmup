...
public class PromotionWithVoucherTest extends AbstractPromotionServiceTest
{
	...
	@Test
	@Ignore("This is just an example to show the effect of Jalo-level calculation while having unsaved changes at model level!")
	public void testUnsavedCartEntryVsJaloCalculate() throws JaloPriceFactoryException
	{
		final Cart c = modelService.getSource(cart);

		final CartEntryModel entry = cartService.addNewEntry(cart, product1, 2, product1.getUnit());
		...
		// dirty hack --> calculate via Jalo .. oh well
		c.recalculate();
		assertTrue(c.isCalculatedAsPrimitive());

		// save via SL
		modelService.save(cart);
		assertFalse(modelService.isNew(entry));
		assertTrue(modelService.isUpToDate(cart));
		assertTrue(modelService.isUpToDate(entry));

		// now the cart shall not be calculated any more since we've added a new entry!!!
		assertEquals(Boolean.FALSE, cart.getCalculated());
		assertFalse(c.isCalculatedAsPrimitive());
	}
