...
public class RuleEngineCompilePublishJobPerformableUnitTest
{
	...
	public void setUp()
	{
		...
		when(ruleEngineCronJob.getSourceRules()).thenReturn(sourceRules);
		when(ruleEngineCronJob.getEnableIncrementalUpdate()).thenReturn(Boolean.TRUE);
	...
	public void testPerformOk()
	{
		...
		when(ruleMaintenanceService.compileAndPublishRulesWithBlocking(sourceRules, MODULE_NAME, true, cronJobProgressTracker)).thenReturn(result);
