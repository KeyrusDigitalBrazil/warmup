...
import static de.hybris.platform.commercefacades.util.XmlAssertions.assertXPathEvaluatesTo;
 ...
 */
@Component
public abstract class AbstractSubscriptionFacadesSpockTest extends AbstractCommerceFacadesSpockTest {
	private static final Logger LOG = Logger.getLogger(AbstractSubscriptionFacadesSpockTest.class);
	...
	public void verifyProductXml(final String productCode, final String xpath, final String expectedXml) {
		try {
			...
			assertXPathEvaluatesTo("The product XML does not match the expectations:", productXml, xpath, expectedXml,
					"subscriptionfacades/transformation/IgnoreEmptyLists.xsl");
		}
	...
	public void verifyClonedChildCarts(final String originalCartCode, final String clonedCartCode) {
		...
		if (CollectionUtils.isEmpty(clonedMasterCartModel.getChildren())) {
			...
			for (AbstractOrderModel originalChildCart in originalMasterCartModel.getChildren()) {
				for(AbstractOrderModel clonedChildCart in clonedMasterCartModel.getChildren()) {
					if (originalChildCart.getBillingTime().getCode().equals(clonedChildCart.getBillingTime().getCode())) {
						...
						assertEquals("Cloned child cart has wrong [parent] code.", clonedMasterCartModel.getCode(), clonedChildCart.getParent().getCode());
					}
				}
			}
		}
	...
	private void verifyChildCartClone(final CartModel originalChildCart, final CartModel clonedChildCart) {
		...
		assertEquals("Cloned child cart does not have the same [totalPrice] as the original child cart",
				originalChildCart.getTotalPrice(), clonedChildCart.getTotalPrice(), 0.00);
	...
	private void verifyClonedChildCartEntries(final CartModel originalChildCart, final CartModel clonedChildCart) {
		...
		for (int i = 0; i < originalChildCart.getEntries().size(); i++) {
			...
			assertEquals(String.format(
					...
					clonedCartEntryModel.getBasePrice(), 0.0);
			...
			assertEquals(String.format(
					...
					clonedCartEntryModel.getTotalPrice(), 0.0);
