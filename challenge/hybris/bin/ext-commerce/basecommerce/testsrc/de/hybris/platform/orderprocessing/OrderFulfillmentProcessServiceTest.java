...
public class OrderFulfillmentProcessServiceTest extends AbstractOrderManagementTest
{
	...
	public void testStartFulfillmentProcess() throws Exception
	{
		...
		process.getCode();
		final BusinessProcessModel bussinessProcess = businessProcessServiceDao.findProcessByName(process.getCode());
		Assert.assertEquals("OrderProcess is not as expected", process, bussinessProcess);
		Assert.assertEquals("Order related to process is not as expected", getOrder(),
				((OrderProcessModel) bussinessProcess).getOrder());
