...
public class B2BCostCenterPopulatorTest
{
	...
	public void shouldPopulate()
	{
		...
		Assert.assertEquals("costCenterData.getCurrency().getIsocode() should be " + CUR_ISOCODE, CUR_ISOCODE,
				costCenterData.getCurrency().getIsocode());
		Assert.assertEquals("costCenterData.getCurrency().getIsocode() should be " + costCenter.getActive(), costCenter.getActive(),
				Boolean.valueOf(costCenterData.isActive()));
		Assert.assertTrue("size of costCenterData.getUnit().getAddresses() should be 1",
				costCenterData.getUnit().getAddresses().size() == 1);
	}
}
