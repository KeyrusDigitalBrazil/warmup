<%@ page import="java.util.*, de.hybris.platform.util.*,com.sap.security.core.server.csi.XSSEncoder" %>
...
<%
  ...
  while( parameterList.hasMoreElements() )
  {
    ...
    {
      out.println("<tr><td>"+ XSSEncoder.encodeHTML(sName) +"</td><td>");
      if (request.getParameter( sName ).length() > 55)
      {
	      out.println(XSSEncoder.encodeHTML(request.getParameter( sName ).substring(0,50)) + " ...  <i>(first 50 chars of the generated xml file as Base64 encoded string)</i></td></tr>" );
      }
      else
      {
    	  out.println(XSSEncoder.encodeHTML(request.getParameter( sName )) + "</td></tr>" );
      }       
    }
      ...
      for( int i=0; i<sMultiple.length; i++ )
        out.println("<tr><td>"+ XSSEncoder.encodeHTML(sName) + "[" + i + "] </td><td>" + XSSEncoder.encodeHTML(sMultiple[i]) + "</td></tr>" );
