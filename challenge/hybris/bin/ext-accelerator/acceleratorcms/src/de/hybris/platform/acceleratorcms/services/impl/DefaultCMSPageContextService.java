...
{
	private static final Logger LOG = Logger.getLogger(DefaultCMSPageContextService.class);

	private static final String PARAMETER_REQUEST_NOT_NULL_MSG = "Parameter request must not be null";
	private static final String STOREFRONT_PREVIEW_ENABLED = "storefront.preview.enabled";
	...
	protected PreviewDataModel getPreviewDataModel(final ServletRequest request)
	{
		...
		if (StringUtils.isNotBlank(previewTicketId) && Config.getBoolean(STOREFRONT_PREVIEW_ENABLED, true))
		{
	...
	protected Map<String, ContentSlotData> getPositionsToContentSlots(final AbstractPageModel page)
	{
		...
		final Collection<ContentSlotData> slotModels = getCmsPageService().getContentSlotsForPage(page,
				getCmsPreviewService().getPagePreviewCriteria());
