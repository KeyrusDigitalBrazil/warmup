...
@UnitTest
public class C4CQuoteCancelEventListenerTest
{

	@InjectMocks
	private C4CQuoteCancelEventListener c4cQuoteCancelEventListener = new C4CQuoteCancelEventListener();
   ...
   public void setUp() {
	...
	public void testOnEvent()
	{
		...
		C4CQuoteCancelEvent event  =  new C4CQuoteCancelEvent(quote, user, QuoteUserType.BUYER);
		...
		when(businessProcessService.createProcess(Mockito.anyString(), Mockito.anyString(), Mockito.anyMap())).thenReturn(quoteCancelledProcessModel);
		
		
		doNothing().when(businessProcessService).startProcess(Mockito.any(QuoteProcessModel.class));
		
		c4cQuoteCancelEventListener.onEvent(event);
