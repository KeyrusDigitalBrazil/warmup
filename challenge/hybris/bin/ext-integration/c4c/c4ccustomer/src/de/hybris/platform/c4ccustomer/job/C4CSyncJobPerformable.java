...
public abstract class C4CSyncJobPerformable extends AbstractJobPerformable<Y2YSyncCronJobModel>
{
	 ...
	 */
	protected C4CBatchingCollector getSplittingCollector(final String syncExecutionId, final String collectorId)
	{
		final C4CBatchingCollector result = new C4CBatchingCollector(
			...
			final Set<StreamConfigurationModel> configurations)
	{
		...
		final C4CBatchingCollector customerCollector = getSplittingCollector(syncExecutionId, CustomerModel._TYPECODE + '-' + qualifier);
		final C4CBatchingCollector addressCollector = getSplittingCollector(syncExecutionId, AddressModel._TYPECODE + '-' + qualifier);
	...
	protected boolean isChangesDetected(List<MediasForType> allMedias) {
		return allMedias.stream().anyMatch((m) -> {
			return CollectionUtils.isNotEmpty(m.getMediaPks());
		});
	}
