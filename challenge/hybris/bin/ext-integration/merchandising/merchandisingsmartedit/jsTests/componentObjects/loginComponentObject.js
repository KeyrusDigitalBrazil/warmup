...
module.exports = {
    ...
    logoutUser: function() {
        browser.switchToParent().then(function() {
            ...
            browser.waitUntil(protractor.ExpectedConditions.elementToBeClickable(this.logoutButton()), "Timed out waiting for logout button");
            return browser.click(this.logoutButton());
        }.bind(this));
    },

    loginAsUser: function(username, password) {
        return browser.waitUntil(protractor.ExpectedConditions.elementToBeClickable(this.mainLoginUsernameInput()), "Timed out waiting for username input").then(function() {
